...
def getRigidTransformation(frame1, frame2):
	...
	warp_matrix = cv2.estimateRigidTransform(
	    (frame1 * 255 + 0.5).astype('uint8'),
	    (frame2 * 255 + 0.5).astype('uint8'), False)
	return warp_matrix
...
def estimateCC(frame1, frame2):
	...
	criteria = (cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT, max_iterations,
	            termination_eps)
	(cc, warp_matrix) = cv2.findTransformECC(frame1, frame2, warp_matrix,
	                                         warp_mode, criteria)
...
def getTransformECC(frame1,
                    frame2,
                    warp_mode=None,
                    max_iterations=None,
                    termination_eps=None,
                    init_warp_matrix=None):
	...
	criteria = (cv2.TERM_CRITERIA_EPS | cv2.TERM_CRITERIA_COUNT, max_iterations,
	            termination_eps)
	(cc, warp_matrix) = cv2.findTransformECC(frame1, frame2, init_warp_matrix,
	                                         warp_mode, criteria)
	return (cc, warp_matrix)
...
def gradientTransformECC(frame1,
                         frame2,
                         warp_mode=None,
                         max_iterations=None,
                         termination_eps=None,
	...
	return getTransformECC(g1, g2, warp_mode, max_iterations, termination_eps,
	                       init_warp_matrix)
...
def laplacianTransformECC(frame1,
                          frame2,
                          warp_mode=None,
                          max_iterations=None,
                          termination_eps=None,
	...
	return getTransformECC(l1, l2, warp_mode, max_iterations, termination_eps,
	                       init_warp_matrix)
...
def warpFrame(frame, warp_matrix, warp_mode, interpolation_mode=None):
	...
	if warp_mode == cv2.MOTION_HOMOGRAPHY:
		frameAligned = cv2.warpPerspective(
		    frame,
		    warp_matrix, (cols, rows),
		    flags=interpolation_mode + cv2.WARP_INVERSE_MAP)
	else:
		frameAligned = cv2.warpAffine(
		    frame,
		    warp_matrix, (cols, rows),
		    flags=interpolation_mode + cv2.WARP_INVERSE_MAP)
	return frameAligned
